...
Invoke-Checked $PythonExecutable ($launcherArguments + @("-I", "-c", $pythonProbe))
Invoke-Checked $PythonExecutable ($launcherArguments + @("-I", "-m", "venv", $Environment))
$python = Join-Path $Environment "Scripts\python.exe"
Invoke-Checked $python @("-I", "-m", "pip", "install", "pip==25.3")

if ($Backend -eq "BandIt") {
    Invoke-Checked $python @(
        "-I", "-m", "pip", "install", "torch==2.8.0", "torchaudio==2.8.0",
        "--index-url", "https://download.pytorch.org/whl/cu128"
    )
    Invoke-Checked $python @(
        "-I", "-m", "pip", "install", "numpy==1.26.4", "scipy==1.15.3",
