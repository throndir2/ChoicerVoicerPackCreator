...
class BackingDialog(QDialog):
    def __init__(
        ...
        *,
        ...
        self.change_mode_button = QPushButton("Cancel and choose mode...")
        self.change_mode_button.setToolTip(
            "Stop this request before choosing a different generation mode. Existing audio is kept."
        )
        self.change_mode_button.setVisible(False)
        if on_change_mode is not None:
            self.change_mode_button.clicked.connect(on_change_mode)
        buttons.addWidget(self.change_mode_button)
