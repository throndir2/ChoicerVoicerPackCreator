...
class SpeakerWorker(JobWorker):
    ...
    def __init__(self, manager, media, clips, *, allow_download: bool) -> None:
        super().__init__()
...
class SpeakerMatchingControls(QWidget):
    ...
    def __init__(self, editor: ProjectEditor) -> None:
        ...
        self._result: SpeakerResult | None = None
        self._outcome = ""
        ...
        self.enabled_check.setToolTip(
            "After you finish naming a segment, compare voices locally and fill confident "
            "matches in unassigned segments. Manual names and cleared segments are preserved."
        )
    ...
    def changed(self) -> None:
        ...
        if self.worker is not None:
            self._pending = True
        if self._activated and not self._typing and not self._paused:
            self._timer.start(900)
    ...
    def _start(self) -> None:
        if (
            ...
            or self._typing or self._paused or self._pending_consent
        ):
            return
        ...
        self._request = _Request(
            self._generation, self.editor.session.source_token(), references, targets, clips,
        )
        self._pending = False
        self._result = None
        self._outcome = ""
        ...
        try:
            ...
            self._paused = True
            self.status.setText(f"Speaker matching could not start: {error}")
            return
        ...
        worker.configure_job(
            ...
            "speakers", "Match unassigned speakers", resource_class="cpu",
            read_paths=tuple({Path(clip.path) for clip in clips}),
            resource_keys=("speaker-matching-inference",),
        ...
        self.status.setText("Speaker matching queued. You can keep editing.")
        worker.start()
    ...
    def _completed(self, result: object) -> None:
        if not isinstance(result, SpeakerResult):
    ...
    def _finished(self) -> None:
        ...
        self._update_buttons()
        if self._pending and not self._paused and not self._typing:
            self._timer.start(900)
