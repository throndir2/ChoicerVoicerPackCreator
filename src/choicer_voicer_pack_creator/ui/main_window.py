...
class ProjectEditor(QWidget):
    ...
    def _restore_layout_state(self) -> None:
        ...
        try:
            ...
            playback_state = self.settings.value("layout/playbackSplitterV1")
            if playback_state is None or not self.playback_splitter.restoreState(playback_state):
                controls_height = self.playback_splitter.widget(1).minimumSizeHint().height()
                self.playback_splitter.setSizes([
                    max(96, self.playback_splitter.height() - controls_height - 1),
                    controls_height,
                ])
            self.playback_splitter.setHandleWidth(1)
    ...
    def _save_layout_state(self) -> None:
        ...
        self.settings.setValue("layout/playbackSplitterV1", self.playback_splitter.saveState())
...
class MainWindow(QMainWindow):
    ...
    def _view_state(self, editor: ProjectEditor) -> dict:
        return {
            ...
            "playback_sizes": editor.playback_splitter.sizes(),
    ...
    def _restore_view(editor: ProjectEditor, view: dict) -> None:
        ...
        for key, splitter in (
            ...
            ("playback_sizes", editor.playback_splitter),
